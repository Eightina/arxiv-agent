...
async def main(
    ...
    store = SimpleEngine.from_docs(input_files=["/home/int.orion.que/dev/my_programs/arxiv-agent/rag/categories.txt"])
    team = Team()
    team.hire(
        [
            SimpleCrawler(),
            Summarizer(outputPath=outputPath, store=store),
        ]
    )
